...
stats_basicas <- variables_numericas %>%
...
Primero, analicemos la presencia de valores NA en nuestras variables numéricas:

```{r na_analysis, echo=FALSE}
# Contar NAs por variable
na_counts <- colSums(is.na(variables_numericas))
na_percentages <- round(100 * na_counts / nrow(variables_numericas), 2)

na_summary <- data.frame(
  Variable = names(na_counts),
  `Cantidad de NAs` = na_counts,
  `Porcentaje de NAs` = na_percentages
)

kable(na_summary, 
      caption = "Análisis de Valores Faltantes (NA)")
```
